...
for(i in seq(1, max(folds))){
    ...
    y.real <- y[folds == i]
    print('log Loss for fold ', i, ' :', paste0(logLoss(y.real, y.pred)), '.', quote = FALSE)
}
...
overexpression.res <- data.frame(perturbationID = overexpression[, 1], pred = predicted.overexpression)
